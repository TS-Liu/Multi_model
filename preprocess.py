...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, srcm_corpus, tgtm_corpus, tgt_mp_corpus, fields,
                                           corpus_type, opt):
    """
    ...
    with codecs.open(src_corpus, "r", encoding="utf-8") as fsrc, codecs.open(tgt_corpus, "r", encoding="utf-8") as ftgt:
        with codecs.open(srcm_corpus, "r", encoding="utf-8") as fsrcm, codecs.open(tgtm_corpus, "r", encoding="utf-8") as ftgtm, \
                codecs.open(tgt_mp_corpus, "r", encoding="utf-8") as ftgt_mp:
            logger.info("Reading source and target files: %s %s."
                        % (src_corpus, tgt_corpus))
            src_data = fsrc.readlines()
            tgt_data = ftgt.readlines()
            srcm_data = fsrcm.readlines()
            tgtm_data = ftgtm.readlines()
            tgt_mp_data = ftgt_mp.readlines()
            ...
            for x in range(num_shards):
                ...
                f.close()
                f = codecs.open(srcm_corpus + ".{0}.txt".format(x), "w",
                                encoding="utf-8")
                f.writelines(
                    srcm_data[x * opt.shard_size: (x + 1) * opt.shard_size])
                f.close()
                f = codecs.open(tgtm_corpus + ".{0}.txt".format(x), "w",
                                encoding="utf-8")
                f.writelines(
                    tgtm_data[x * opt.shard_size: (x + 1) * opt.shard_size])
                f.close()
                f = codecs.open(tgt_mp_corpus + ".{0}.txt".format(x), "w",
                                encoding="utf-8")
                f.writelines(
                    tgt_mp_data[x * opt.shard_size: (x + 1) * opt.shard_size])
                f.close()
            ...
            if len(src_data) > num_written:
                ...
                f.close()
                f = codecs.open(srcm_corpus + ".{0}.txt".format(num_shards),
                                'w', encoding="utf-8")
                f.writelines(
                    srcm_data[num_shards * opt.shard_size:])
                f.close()
                f = codecs.open(tgtm_corpus + ".{0}.txt".format(num_shards),
                                'w', encoding="utf-8")
                f.writelines(
                    tgtm_data[num_shards * opt.shard_size:])
                f.close()
                f = codecs.open(tgt_mp_corpus + ".{0}.txt".format(num_shards),
                                'w', encoding="utf-8")
                f.writelines(
                    tgt_mp_data[num_shards * opt.shard_size:])
                f.close()

    src_list = sorted(glob.glob(src_corpus + '.*.txt'))
    tgt_list = sorted(glob.glob(tgt_corpus + '.*.txt'))
    srcm_list = sorted(glob.glob(srcm_corpus + '.*.txt'))
    tgtm_list = sorted(glob.glob(tgtm_corpus + '.*.txt'))
    tgt_mp_list = sorted(glob.glob(tgt_mp_corpus + '.*.txt'))
    ...
    for index, src in enumerate(src_list):
        ...
        dataset = inputters.build_dataset(
            ...
            srcm_path=srcm_list[index],
            tgtm_path=tgtm_list[index],
            tgt_mp_path=tgt_mp_list[index],
            src_dir=opt.src_dir,
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        tgt_m = opt.train_tgt_m
        src_m = opt.train_src_m
        tgt_m_p = opt.train_tgt_m_p
    else:
        src_corpus = opt.valid_src
        tgt_corpus = opt.valid_tgt
        src_m = opt.valid_src_m
        tgt_m = opt.valid_tgt_m
        tgt_m_p = opt.valid_tgt_m_p
